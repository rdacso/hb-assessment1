...
def print_list(items):
    ...
    #for loop that prints each item in a list
    for item in items:
        print item 
...
def long_words(words):
    ...
    #empty string to add values to
    word_list = []

    #for loop that iterates through the list and appends words with more than 4 characters to the new list.
    for item in words: 
        if len(item) > 4:
...
def n_long_words(words, n):
    ...
    #variable assigned to an empty list
    words_of_certain_length = []

    #for loop that iterates through each item of a list, looks at the length, and evaluates if it is more than n characters. If its more than n characters it is appended to the new list.
    for item in words:
        if len(item) > n:
            words_of_certain_length.append(item)
    return words_of_certain_length
...
def halvesies(numbers):
    ...
    divided_in_half = []
    for item in numbers:
        division = float(item)/float(2)
        divided_in_half.append(division)
    return divided_in_half
...
def sum_numbers(numbers):
    ...
    Any empty list should return the sum of zero::
        ...
        0
    """
    #have to define a variable to use it in the for loop
    sum = 0
    #for each number in the list, iterate through and add each item to the one before it.
    for unit in numbers:
        sum += unit
    return sum
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    #skipping for now so I can read it through again for better understanding.
...
def duplicates(items):
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'apple', 'berry']
    """
    stored_duplicates = set([])
    for word in items:
        if items.count(word) > 1:
            stored_duplicates.add(word)
        new_duplicate_list = list(stored_duplicates)
    return new_duplicate_list
